        ...
        success: function(result) {
            for (var i in result.data) {
                ...
                for(var j in page.Status){
                    ...
                    data.push(new Submit(1, sub.uname, sub.pid, StringToDate(sub.date), sub.status));
                }
            }
        },
        error: function() {
    ...
    $.ajax({
        ...
        success: function(result) {
            for (var key in result.users) {
                ...
                data[team.username] = new Team(team.username, team.nickname, null, team.official ,team.gender);
            }
        },
        error: function() {
            alert("获取Team数据失败");
 ...
 */
function Team(teamId, teamName, teamMember, official, gender) {
    this.teamId = teamId; //队伍ID
...
function TeamCompare(a, b) {
	...
	return a.submitList.length>b.submitList.length?-1:1;
}
...
function Board(problemCount, medalCounts, startTime, freezeBoardTime) {
    ...
    this.medalStr = ["gold", "silver", "bronze","gend","topg","rate","unrate"];
    ...
    for (var i = 0; i < problemCount/2; i++)
        this.problemList.push(String.fromCharCode(ACode + i)+"_1");
    for (var i = 0; i < problemCount/2; i++)
        this.problemList.push(String.fromCharCode(ACode + i)+"_2");
...
Board.prototype.showInitBoard = function() {
    ...
    var teamPer = 6; //Team列宽度百分比
    var solvedPer = 4; //Solved列宽度百分比
    var penaltyPer = 6; //Penalty列宽度百分比
...
///////////
		var rnk=1;
		medal=-1;
		for(var kk=0;kk<this.teamCount;++kk)
		{
			if(kk==i)
				continue;
			var jj=this.teamNowSequence[kk];
			if(jj.official)
				if(TeamCompare(jj,team)==-1)
					++rnk;
		}
		for(var kk = this.medalRanks.length - 1 ;kk>=0;--kk)
			if(rnk<=this.medalRanks[kk])
				medal=kk;
		var tpg=1;
		if(!team.official)
			medal=6;
		else if(medal==-1&&team.gender)
		{
			for(var kk=0;kk<this.teamCount;++kk)
			{
				if(kk==i)
...
////////////
        ...
        var rankHTML;
		if(team.official)
			rankHTML = "<th class=\"rank\" width=\"" + rankPer + "%\">" + rnk + "</th>";
		else if(1)
...
//////???????????
				...
				if(t.official)
					rnk=1;
				for(var kk=0;kk<thisBoard.teamCount;++kk)
				{
					if(kk==i)
						continue;
					var jj=thisBoard.teamNextSequence[kk];
					if(jj.official)
						if(TeamCompare(jj,t)==-1)
							++rnk;
				}
				for(var kk = thisBoard.medalRanks.length - 1 ;kk>=0;--kk)
					if(rnk<=thisBoard.medalRanks[kk])
						medal=kk;
				var tpg=1;
				if(!t.official)
					medal=6;
				else if(medal==-1&&t.gender)
				{
					for(var kk=0;kk<thisBoard.teamCount;++kk)
					{
						if(kk==i)
